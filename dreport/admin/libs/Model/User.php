	...
	public function Login($username,$password)
	{
		...
		try {
			...
			if (password_verify($password, $hash))
			{
                ...
                //$statistic-> saveLog('101',$username,'Login sucsess. User:'.$username);
			}
			else
			{
				// THE USERNAME WAS FOUND BUT THE PASSWORD DIDN'T MATCH
				$result = false;
                //$this->StatisticController->saveLog('102',$username,'Login fail. User:'.$username);
			}
			...
			$result = false;
            //$ ->saveLog('102',$username,'Login fail. User:'.$username);
		}
