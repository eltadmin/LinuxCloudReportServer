<?php
	$this->assign('title','DREPORTS | Statisticses');
...
<h1>
	<i class="icon-th-list"></i> Statisticses
	...
	<script type="text/template" id="statisticCollectionTemplate">
		...
		<% items.each(function(item) { %>
			<tr id="<%= _.escape(item.get('id')) %>">
				...
				<td><%= _.escape(item.get('opertype') || '') %></td>
				<td><%= _.escape(item.get('operid') || '') %></td>
				<td><%if (item.get('datetime')) { %><%= _date(app.parseDate(item.get('datetime'))).format('MM.DD.YYYY hh:mm:ss') %><% } else { %>NULL<% } %></td>
