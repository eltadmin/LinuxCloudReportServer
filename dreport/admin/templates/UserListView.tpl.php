<?php
	$this->assign('title','DREPORTS | Users');
	...
	<script type="text/template" id="userCollectionTemplate">
		...
		<thead>
			<tr>
				...
				<th id="header_RoleId">Role Id<% if (page.orderBy == 'RoleId') { %> <i class='icon-arrow-<%= page.orderDesc ? 'up' : 'down' %>' /><% } %></th>
...
-->
		...
		<% items.each(function(item) { %>
			<tr id="<%= _.escape(item.get('id')) %>">
				...
				<td><%= _.escape(item.get('roleId') || '') %></td>
