<?php
	$this->assign('title','DREPORTS | Roles');
	...
	<script type="text/template" id="roleCollectionTemplate">
		...
		<thead>
			<tr>
				...
				<th id="header_CanAdmin">Can Admin<% if (page.orderBy == 'CanAdmin') { %> <i class='icon-arrow-<%= page.orderDesc ? 'up' : 'down' %>' /><% } %></th>
				<th id="header_CanEdit">Can Edit<% if (page.orderBy == 'CanEdit') { %> <i class='icon-arrow-<%= page.orderDesc ? 'up' : 'down' %>' /><% } %></th>
				<th id="header_CanWrite">Can Write<% if (page.orderBy == 'CanWrite') { %> <i class='icon-arrow-<%= page.orderDesc ? 'up' : 'down' %>' /><% } %></th>
<!-- UNCOMMENT TO SHOW ADDITIONAL COLUMNS
				<th id="header_CanRead">Can Read<% if (page.orderBy == 'CanRead') { %> <i class='icon-arrow-<%= page.orderDesc ? 'up' : 'down' %>' /><% } %></th>
-->
			</tr>
		</thead>
		<tbody>
		<% items.each(function(item) { %>
			<tr id="<%= _.escape(item.get('id')) %>">
				...
				<td><%= _.escape(item.get('canAdmin') || '') %></td>
				<td><%= _.escape(item.get('canEdit') || '') %></td>
				<td><%= _.escape(item.get('canWrite') || '') %></td>
<!-- UNCOMMENT TO SHOW ADDITIONAL COLUMNS
				<td><%= _.escape(item.get('canRead') || '') %></td>
-->
			</tr>
		<% }); %>
		</tbody>
		...
		<form class="form-horizontal" onsubmit="return false;">
			<fieldset>
				...
				<div id="canAdminInputContainer" class="control-group">
					...
					<div class="controls inline-inputs">
						<input type="text" class="input-xlarge" id="canAdmin" placeholder="Can Admin" value="<%= _.escape(item.get('canAdmin') || '') %>">
						<span class="help-inline"></span>
					</div>
				</div>
				<div id="canEditInputContainer" class="control-group">
					...
					<div class="controls inline-inputs">
						<input type="text" class="input-xlarge" id="canEdit" placeholder="Can Edit" value="<%= _.escape(item.get('canEdit') || '') %>">
						<span class="help-inline"></span>
					</div>
				</div>
				<div id="canWriteInputContainer" class="control-group">
					...
					<div class="controls inline-inputs">
						<input type="text" class="input-xlarge" id="canWrite" placeholder="Can Write" value="<%= _.escape(item.get('canWrite') || '') %>">
						<span class="help-inline"></span>
					</div>
				</div>
				<div id="canReadInputContainer" class="control-group">
					...
					<div class="controls inline-inputs">
						<input type="text" class="input-xlarge" id="canRead" placeholder="Can Read" value="<%= _.escape(item.get('canRead') || '') %>">
						<span class="help-inline"></span>
					</div>
				</div>
			</fieldset>
