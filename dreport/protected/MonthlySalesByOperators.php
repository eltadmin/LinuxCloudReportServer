...
if (!$str) {
    ...
    if ($rptdata == null && json_last_error() !== JSON_ERROR_NONE) {
        ...
        if ($rptdata->ResultCode == 0) {
            ...
            $TurnoverData = '';
            $TurnoverCurrency = '';
            if (!empty($rptdata->MonthlySalesByOperator)) {
                $chartBars = count($rptdata->MonthlySalesByOperator);
            }
foreach ($rptdata->MonthlySalesByOperator as $item) {
...
<body>
    ...
    <div class="login-card">
        <div class="login-help">
            ...
            if ($_SESSION['lang'] == 'bg') {
                echo '&nbsp;•&nbsp;<a href="monthlysalesbyoperator.php?lang=en"><img src="images/en.png" /></a>';
            } else {
                echo '&nbsp;•&nbsp;<a href="monthlysalesbyoperator.php?lang=bg"><img src="images/bg.png" /></a>';
            }
        ...
        <h1 align="center"><?php echo $lang["rptMonthlybyOperator"]; ?></h1>
        <h4 align="center">
            <?php
        ...
        tooltips: {
            ...
            callbacks: {
                label: function(tooltipItem, data) {
                    return rptRevenueChartLabel + ':' + rawData[tooltipItem.index].toLocaleString(); 
                }
            }
        }
    }
...
window.onload = function() {
    ...
    ctx.canvas.addEventListener('click', function(evt) {
        var activePoint = window.myLine.getElementAtEvent(evt);
        if (activePoint.length) {
            var label = window.myLine.data.labels[activePoint[0]._index];
            var value = window.myLine.data.datasets[0].data[activePoint[0]._index];
            // Show tooltip at click position
            window.myLine.tooltip._active = [activePoint[0]];
