...
//    $C_SQL  = '{ "Id":"DatabaseId", "Pass":"1234", "PluTurnover": { "Type":"Query", "SQL":"select first 10 S.SPLU_NAME, sum(S.SPLU_SOLDQUANT) \"SPLU_SOLDQUANT\", sum(RoundTo((S.SPLU_SELLPRICE * S.SPLU_SOLDQUANT),2)+RoundTo((S.SPLU_SOLDQUANT * S.SPLU_SELLPRICE * S.SPLU_SELLDISCOUNT/100), 2)) as \"TURNOVER\", C.CURR_SIGN from SALES_PLUES S left join N_CURRENCY C on S.SPLU_SELLCURRENCY = C.CURR_ID where S.SPLU_DATETIME between START_DATE and END_DATE group by 1, 4 order by 3 desc" }}';
    ...
    $C_SQL .= 'where (cast(S.SPLU_DATETIME - cast(TIMEOFFSET as float)/24 as date) BETWEEN PARAMDTSTART AND PARAMDTEND) and (S.SPLU_REVOKED_ = 0)';
...
*/
       ...
       if ($_SESSION['lang'] == 'bg'){
         echo '&nbsp;•&nbsp;<a href="monthpluturnover.php?lang=en"><img src="images/en.png" /></a>';
       } else {
         echo '&nbsp;•&nbsp;<a href="monthpluturnover.php?lang=bg"><img src="images/bg.png" /></a>';
       }
    ...
    <h1 align="center"><?php 
		echo $lang["rptMonthPluTurnoverTitle"]; 
		$forMonth = new DateTime($rptEndDate);
		echo strftime(" %b %Y", $forMonth->getTimestamp());		
	?></h1>
    <h4 align="center">
    <?php
