...
$C_SQL = '{"Id":"DatabaseId","Pass":"1234","PLUESQuery":{"Type":"Query","SQL":"SELECT CASE WHEN A.PAC_OWNER = \'\' THEN \'POS\' ELSE A.PAC_OWNER END AS PAC_OWNER, COUNT(A.PAC_CARDID) FROM PAC_ACCESSLIST A WHERE ((A.PAC_VALIDFROMD is NULL) OR (A.PAC_VALIDFROMD <= CURRENT_DATE)) AND ((A.PAC_VALIDTOD is NULL) OR (A.PAC_VALIDTOD >= CURRENT_DATE)) AND ((A.PAC_VALIDFROMT is NULL) OR (A.PAC_VALIDFROMT <= CURRENT_TIME)) AND ((A.PAC_VALIDTOT is NULL) OR (A.PAC_VALIDTOT >= CURRENT_TIME)) AND (A.PAC_DISABLED = 0) GROUP BY 1"}}';
 ...
 ?>
    <div class="login-card" >
		...
		<div class="login-help">
            ...
            <h1><?php echo $lang['objTicketAval']; ?></h1> 
               ...
               <table border="0" cellpadding="10" cellspacing="0" style="width: 100%; border-collapse: separate; text-align: center;">
                    ...
                    <tbody>
                        <?php foreach ($combinedItems as $item): ?>
                            <tr>
                                <td style="border-left: 1px solid #36752d; border-right: 1px solid #36752d; padding: 8px; font-size: 12px; font-family: Arial, Helvetica, sans-serif;"><?= isset($item['PAC_OWNER']) ? htmlspecialchars($item['PAC_OWNER']) : '' ?></td>
								<td style="border-left: 1px solid #36752d; border-right: 1px solid #36752d; padding: 8px; font-size: 12px; font-family: Arial, Helvetica, sans-serif;"><?= isset($item['COUNT']) ? number_format($item['COUNT'], 0) : '' ?></td>
            ...
            <p style="text-align: center;">
                 <a href="rptlist.php" class="medium color-3 button" style="width:100%;margin-top:10px;">
