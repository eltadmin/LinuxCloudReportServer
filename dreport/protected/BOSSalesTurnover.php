...
<?php
    ...
    $C_SQL  = '{"Id":"DatabaseId","Pass":"1234","CurrentTurnover":{"Type":"Query","SQL":"SELECT CAST (SUP_DATETIME - CAST(TIMEOFFSET AS FLOAT)/24 AS DATE) \"SELL_DATE\", SUM(SUP_SUM) \"SELL_SUM\", CURR_SIGN FROM SUPPLIES JOIN N_CURRENCY ON SUP_CURRENCY = CURR_ID WHERE SUP_TYPE = 2 AND CAST (SUP_DATETIME - CAST(TIMEOFFSET AS FLOAT)/24 AS DATE) = RPT_DATE AND SUP_REVOKED_ = 0 GROUP BY 1, 3 ORDER BY 3 DESC"},';
    $C_SQL .= '"LastTurnover":{"Type":"Query","SQL":"SELECT CAST (SUP_DATETIME - CAST(TIMEOFFSET AS FLOAT)/24 AS DATE) \"SELL_DATE\", SUM(SUP_SUM) \"SELL_SUM\", CURR_SIGN FROM SUPPLIES JOIN N_CURRENCY ON SUP_CURRENCY = CURR_ID WHERE SUP_TYPE = 2 AND CAST (SUP_DATETIME - CAST(TIMEOFFSET AS FLOAT)/24 AS DATE) BETWEEN START_DATE AND END_DATE AND SUP_REVOKED_ = 0 GROUP BY 1, 3 ORDER BY 1"},';    
    $C_SQL .= '"CurrentPayType":{"Type":"Query","SQL":"SELECT DOCUMENT_NAME, SUM(SUP_SUM) \"SUP_SUM\", CURR_SIGN FROM SUPPLIES JOIN N_CURRENCY ON SUP_CURRENCY = CURR_ID JOIN N_DOCUMENTS ON SUP_DOCTYPE = DOCUMENT_ID WHERE SUP_TYPE = 2 AND CAST (SUP_DATETIME - CAST(TIMEOFFSET AS FLOAT)/24 AS DATE) = RPT_DATE AND SUP_REVOKED_ = 0 GROUP BY 1,3 ORDER BY 1"},';
	$C_SQL .= '"Currencies": {"Type":"Query", "SQL": "SELECT CURR_SIGN FROM SUPPLIES JOIN N_CURRENCY ON SUP_CURRENCY = CURR_ID WHERE SUP_TYPE = 2 AND CAST (SUP_DATETIME - CAST(TIMEOFFSET AS FLOAT)/24 AS DATE) BETWEEN START_DATE AND END_DATE AND SUP_REVOKED_ = 0 GROUP BY 1"}}';
	
    define("C_DEBUG", false);
    define("C_RPTNAME", "BOSSalesTurnover");
...
//        $qry = $pDatabase->query("select d_objectpswd from t_devices where d_objectid='".sql_safe($objectid)."';");
        ...
        if (C_DEBUG) {echo "dt: "; var_dump($dt); echo "<br/>";}
         }
		...
		$dt->add(new DateInterval('PT'.($otimeoffset).'H'));
		...
		$rptsql = str_replace('RPT_DATE','\''.$rptdate.'\'',$rptsql);
		
		
        if (C_DEBUG) {echo "report date: "; var_dump($dt); echo "<br/>";}
...
*/
		...
		$TurnoverWithCurrency = array();
		
		$ColorSchemes = array();
		array_push($ColorSchemes, "151,187,205");
		array_push($ColorSchemes, "238,107,107");
		array_push($ColorSchemes, "107,238,116");
		array_push($ColorSchemes, "238,211,107");
		array_push($ColorSchemes, "238,107,185");
          ...
          $pDatabase->logevent(OPER_ERROR,$objectid,'report: '.$rname.' error: '.$lang["rptNoReceivedData"]);
            ...
            if ($rptdata == null && json_last_error() !== JSON_ERROR_NONE) {
              ...
              if ($rptdata->ResultCode == 0) {
				  ...
				  if (C_DEBUG) {echo "CurrentTurnover: "; var_dump($rptdata->CurrentTurnover); echo "<br/>";}	
				  
				  foreach($rptdata->Currencies as $item) {
					  $TurnoverWithCurrency[$item->CURR_SIGN] = '';
				  }
				  
                  $LastTurnoverLabel = '';
				  
				  $ddaattee = $rptstartdate;
				  while($ddaattee < $rptenddate)
				  {			  
					//if (C_DEBUG) {echo "Date: "; var_dump($ddaattee); echo "<br/>";}
			  
					$ymd = DateTime::createFromFormat('Y-m-d', $ddaattee)->format('d.m');
					$LastTurnoverLabel .= '"'.$ymd.'",';

					$sellSum = 0;
					$currSign = '';
					
					$InternalTurnoverWithCurrency = array();
					foreach($rptdata->Currencies as $item) {
					  $InternalTurnoverWithCurrency[$item->CURR_SIGN] = '0.00,';
					}
					
					foreach($rptdata->LastTurnover as $item)
					{
						if(DateTime::createFromFormat('d.m.Y', $item->SELL_DATE )->format('Y-m-d') == $ddaattee)
						{
							$sellSum = $item->SELL_SUM;
							$currSign = $item->CURR_SIGN;
							
							$InternalTurnoverWithCurrency[$currSign] = number_format((float)$sellSum, 2, '.', '').',';
						}
					}
					
					//if (C_DEBUG) {echo "Internal: "; var_dump($InternalTurnoverWithCurrency); var_dump($key); echo "<br/>";}						

					foreach($TurnoverWithCurrency as $key => &$value)
					{
						$TurnoverWithCurrency[$key] .= $InternalTurnoverWithCurrency[$key];
					}
					
					
					$AvgTurnover = $AvgTurnover + (float)$sellSum;
					$ItemsCount++;

					$ddaattee = (new DateTime($ddaattee))->modify('+1 day')->format('Y-m-d');
				  }
				  
				  foreach($TurnoverWithCurrency as &$val){
					  $val = rtrim($val, ",");
				  }
				  
                  $LastTurnoverLabel = rtrim($LastTurnoverLabel, ",");
				  
				  if (C_DEBUG) {echo "LastTurnoverLabel: "; var_dump($LastTurnoverLabel); echo "<br/>";}	
				  if (C_DEBUG) {echo "Currencies array: "; var_dump($TurnoverWithCurrency); echo "<br/>";}	
        ...
        var config = {
            ...
            data: {
				...
				[
					...
					foreach($TurnoverWithCurrency as $key => &$currency)
					{
						$itemIndex = count($ColorSchemes) % ++$iteration;
						
						echo  "{ label: '".$lang["rptRevenueLast5Days"]." (".$key.")', ";
						echo "data: [" .$currency."] ,";
						echo "borderColor : \"rgba(".$ColorSchemes[$itemIndex].",1)\" ,";
						echo "backgroundColor : \"rgba(".$ColorSchemes[$itemIndex].",0.2)\",";
						echo "pointBorderColor : \"#fff\",";
						echo "pointBackgroundColor : \"rgba(".$ColorSchemes[$itemIndex].",1)\",";
						echo "pointHoverBackgroundColor : \"#fff\",";
						echo "pointHoverBorderColor : \"rgba(".$ColorSchemes[$itemIndex].",1)\",";
						echo "hitRadius : 30 },";
					}
...
*/		
       ...
       if ($_SESSION['lang'] == 'bg'){
         echo '&nbsp;•&nbsp;<a href="BOSSalesTurnover.php?lang=en"><img src="images/en.png" /></a>';
       } else {
         echo '&nbsp;•&nbsp;<a href="BOSSalesTurnover.php?lang=bg"><img src="images/bg.png" /></a>';
       }
	...
	{
		echo '<p class="val_money">';
		
		//echo 12345;
		$CurrentTurnover = number_format((float)$item->SELL_SUM, 2, '.', '');
		$CurrentTurnover .= ' '.$item->CURR_SIGN;
		echo $CurrentTurnover;
		
		//var_dump($CurrentTurnover);
    ...
    <h4 align="center">
    <?php
      echo $lang["rptRevenueToDate"].date('d.m.Y', strtotime($rptdate));
     ?>
    </h4>
