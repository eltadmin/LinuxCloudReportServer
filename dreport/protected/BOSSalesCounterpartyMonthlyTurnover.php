...
<?php
	...
	$C_SQL  = '{ "Id":"DatabaseId", "Pass":"1234", "GroupTurnover": { "Type":"Query", "SQL":"SELECT SUP_SUPPLIERBULST, COALESCE(CUST_FIRMNAME, \'\') \"CUST_NAME\", SUM(SUP_SUM) \"SELL_SUM\", CURR_SIGN FROM SUPPLIES JOIN N_CURRENCY ON SUP_CURRENCY = CURR_ID LEFT JOIN CUSTOMERS_DATA ON SUP_SUPPLIERBULST = CUST_BULSTAT WHERE SUP_TYPE = 2 AND CAST (SUP_DATETIME - CAST(TIMEOFFSET AS FLOAT)/24 AS DATE) BETWEEN PARAMDTSTART AND PARAMDTEND AND SUP_REVOKED_ = 0 GROUP BY 1, 2, 4 ORDER BY 2 ASC" } }';
            ...
            if ($rptdata == null && json_last_error() !== JSON_ERROR_NONE) {
              ...
              if ($rptdata->ResultCode == 0) {
                  ...
                  foreach($rptdata->GroupTurnover as $item) {
                   ...
                   $TurnoverData  .= number_format((float)$item->SELL_SUM, 2, '.', '').',';
...
*/
       ...
       if ($_SESSION['lang'] == 'bg'){
         echo '&nbsp;•&nbsp;<a href="BOSSalesCounterpartyMonthlyTurnover.php?lang=en"><img src="images/en.png" /></a>';
       } else {
         echo '&nbsp;•&nbsp;<a href="BOSSalesCounterpartyMonthlyTurnover.php?lang=bg"><img src="images/bg.png" /></a>';
       }
    ...
    <h1 align="center"><?php echo $lang["rptMonthlyTurnoverByCounterparty"]; ?></h1>
    <?php
