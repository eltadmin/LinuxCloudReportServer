...
$pluEcrName = $_POST['pluEcrName'];

$C_SQL = '{"Id":"DatabaseId","Pass":"1234","PLUESQuery":{"Type":"Query","SQL":"UPDATE PLUES SET PLU_ECR_NAME = \'' . $pluEcrName . '\' WHERE PLU_NUMB = \'' . $pluNumb . '\'"}}';
...
$pDatabase->logevent(OPER_COMMAND,$objectid,'report: UpdateEcrName.php New ecrName for plu= '.$pluNumb.' objectid='.$objectid);

if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['pluNumb'], $_POST['pluEcrName'])) {
    $pluNumb = $_POST['pluNumb'];
    $pluEcrName = $_POST['pluEcrName'];
    ...
    $EV_DESCRIPTION = '[МОБИЛНИ СПРАВКИ] [ПРОМЯНА КАСОВО ИМЕ] ОБЕКТ (' . $objectid . ') Оператор (' . $operator . ') направи промяна на касово име(' . $pluEcrName . ') за артикул с артикулен номер(' . $pluNumb . ').';
