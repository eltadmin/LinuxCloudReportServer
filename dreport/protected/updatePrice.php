...
$C_SQL = '{"Id":"DatabaseId","Pass":"1234","PLUESQuery":{"Type":"Query","SQL":"UPDATE PLUES SET PLU_SELL_PRICE = \'' . $sellPrice . '\' WHERE PLU_NUMB = \'' . $pluNumb . '\'"}}';
...
$pDatabase->logevent(OPER_COMMAND,$objectid,'report: UpdatePrice.php New sell price for plu= '.$pluNumb.' objectid='.$objectid);


if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['pluNumb'], $_POST['sellPrice'])) {
    $pluNumb = $_POST['pluNumb'];
    $sellPrice = $_POST['sellPrice'];
    ...
    if ($result === FALSE) {
        ...
        echo "Sell Price updated successfully";

        // Construct the dynamic EV_DESCRIPTION and EV_DATATEXT
        $EV_DESCRIPTION = '[МОБИЛНИ СПРАВКИ] [ПРОМЯНА ЦЕНА] ОБЕКТ (' . $objectid . ') Оператор (' . $operator . ') направи промяна на цена(' . $sellPrice . ') за артикул с артикулен номер(' . $pluNumb . ').';
