...
$C_SQL = '{"Id":"DatabaseId","Pass":"1234","PLUESQuery":{"Type":"Query","SQL":"UPDATE PLUES SET PLU_NAME = \'' . $pluName . '\' WHERE PLU_NUMB = \'' . $pluNumb . '\'"}}';
...
$pDatabase->logevent(OPER_COMMAND,$objectid,'report: UpdateName.php New name for plu= '.$pluNumb.' objectid='.$objectid);


if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['pluNumb'], $_POST['pluName'])) {
    $pluNumb = $_POST['pluNumb'];
    $pluName = $_POST['pluName'];
    ...
    if ($result === FALSE) {
        ...
        $EV_DESCRIPTION = '[МОБИЛНИ СПРАВКИ] [ПРОМЯНА ИМЕ] ОБЕКТ (' . $objectid . ') Оператор(' . $operator . ') направи промяна на име на артикул(' . $pluName . ') с артикулен номер(' . $pluNumb . ').';
