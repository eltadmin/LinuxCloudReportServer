...
<head>
  ...
  .color-1 { background-color: #365c94; }
...
<?php
	...
	$C_SQL  = '{ "Id":"DatabaseId", "Pass":"1234", "GroupTurnover": { "Type":"Query", "SQL":"SELECT SUP_SUPPLIERBULST, COALESCE(CUST_FIRMNAME, \'\') \"CUST_NAME\", SUM(SUP_SUM) \"SELL_SUM\", CURR_SIGN FROM SUPPLIES JOIN N_CURRENCY ON SUP_CURRENCY = CURR_ID LEFT JOIN CUSTOMERS_DATA ON SUP_SUPPLIERBULST = CUST_BULSTAT WHERE SUP_TYPE = 2 AND CAST (SUP_DATETIME - CAST(TIMEOFFSET AS FLOAT)/24 AS DATE) = PARAMDATE AND SUP_REVOKED_ = 0 GROUP BY 1, 2, 4 ORDER BY 2 ASC" } }';
            ...
            if ($rptdata == null && json_last_error() !== JSON_ERROR_NONE) {
              ...
              if ($rptdata->ResultCode == 0) {
                  ...
                  foreach($rptdata->GroupTurnover as $item) {
                   $TurnoverLabel .= '"'.str_replace('"', "'", $item->CUST_NAME).'",';
                   $TurnoverData  .= number_format($item->SELL_SUM, 2, '.', '').',';
                   $TurnoverCurrency = $item->CURR_SIGN;
                   $GroupIdData .= '"'.$item->SUP_SUPPLIERBULST.'",';
                  }
...
*/
       ...
       if ($_SESSION['lang'] == 'bg'){
         echo '&nbsp;•&nbsp;<a href="BOSSalesCounterpartyTurnover.php?lang=en"><img src="images/en.png" /></a>';
       } else {
         echo '&nbsp;•&nbsp;<a href="BOSSalesCounterpartyTurnover.php?lang=bg"><img src="images/bg.png" /></a>';
       }
    ...
    <h1 align="center"><?php echo $lang["rptDailyTurnoverByCounterparty"]; ?></h1>
    <?php
    ...
    <h4 align="center"><?php echo $lang["rptGroupTurnoverCurrentDate"]; ?></h4>
    <br>
        ...
        <td align="center" style="width:60%;"><a href="rptlist.php" class="medium color-5 button" style="width:95%;"><?php echo $lang["btnExit"]; ?></a></td>
           <?php
            if ($expiredate >= time()){
			   ...
			   echo '<td align="center"><a href="#" onclick="show_exp()" class="medium color-5 button">>></a></td>';
			}
