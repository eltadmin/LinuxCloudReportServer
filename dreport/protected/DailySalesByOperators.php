...
<head>
  ...
  <style>
    ...
    .color-1 { background-color: #4571b5; }
...
define("C_RPTNAME", "dailysalesbyoperator");
define("C_TIMEOFFSET", 0);

// param START_DATE, END_DATE
$C_SQL  = '{ "Id":"DatabaseId", "Pass":"1234", "DailySalesByOperator": { "Type":"Query", "SQL":"SELECT SALES_BON.SELL_OPERATOR as \\"Operator ID\\", SALES_BON.SELL_OPERNAME as \\"Operator Name\\", N_CURRENCY2.CURR_ABR as \\"Currency\\", SUM(SALES_BON.SELL_SUM) as \\"Total Sales Amount\\" FROM SALES_BON LEFT JOIN N_CURRENCY N_CURRENCY2 ON N_CURRENCY2.CURR_ID = SALES_BON.SELL_CURRTYPE WHERE SALES_BON.SELL_DATETIME >= START_DATE AND SALES_BON.SELL_DATETIME < END_DATE AND SALES_BON.SELL_REVOKED_ = 0 GROUP BY 1, 2, 3 ORDER BY 4 DESC" } }';
...
$rptsql = str_replace('START_DATE', '\'' . $start_date->format('Y-m-d 00:00:00') . '\'', $rptsql);
...
if (!$str) {
    ...
    if ($rptdata == null && json_last_error() !== JSON_ERROR_NONE) {
        ...
        if ($rptdata->ResultCode == 0) {
            ...
            $TurnoverData = '';
            $TurnoverCurrency = '';
            if (!empty($rptdata->DailySalesByOperator)) {
                $chartBars = count($rptdata->DailySalesByOperator);
            }
foreach ($rptdata->DailySalesByOperator as $item) {
    if (isset($item->{"OPERATOR ID"}) && isset($item->{"TOTAL SALES AMOUNT"}) && isset($item->{"CURRENCY"})) {
        $TurnoverLabel .= '"' . str_replace('"', "'", $item->{"OPERATOR NAME"}) . '",';
        $TurnoverData  .= number_format($item->{"TOTAL SALES AMOUNT"}, 2, '.', '') . ',';
        $TurnoverCurrency = $item->{"CURRENCY"};
    }
}
...
<div class="login-card">
    <div class="login-help">
        ...
        if ($_SESSION['lang'] == 'bg') {
            echo '&nbsp;•&nbsp;<a href="dailysalesbyoperator.php?lang=en"><img src="images/en.png" /></a>';
        } else {
            echo '&nbsp;•&nbsp;<a href="dailysalesbyoperator.php?lang=bg"><img src="images/bg.png" /></a>';
        }
    ...
    <h1 align="center"><?php echo $lang["rptDailyByOperator"]; ?></h1>
    <h4 align="center">
        <?php
        $start_date_display = $start_date->format('d.m.Y');
        echo $lang["rptGroupTurnoverToDate"] . $start_date_display;
        ?>
    </h4>
        ...
        <table style="width:100%; line-height:22px;">
            <tr>
                ...
                <td>
                    ...
                    if ($expiredate < time()) {
                        // show back button
                        echo '<center><a href="' . $_SERVER['PHP_SELF'] . '" class="medium color-5 button" style="width:95%;">' . $lang["btnBack"] . '</a></center>';
                    }
                    ?>
                </td>
                <td></td>
...
<script>
    var config = {
        type: 'horizontalBar',
        data: {
            labels: [<?php echo $TurnoverLabel; ?>],
            datasets: [{
                label: <?php echo '"' . $lang["rptRevenueChartLabel"] . '",'; ?>
                data: [<?php echo $TurnoverData; ?>],
    ...
    <?php
        if ($expiredate > time()) {
            ...
            //echo 'window.location.href = "dailysalesbyoperator.php?id=';
            //echo $objectid;
            //if ($rptdate <> '') {
            //    echo '&date=' . $rptdate;
            //}
            //echo '&gname=" +activePoints[0]._view.label+';
            //echo "'&gval='+config.data.datasets[activePoints[0]._index];";
            echo '} };';
        } else {
            echo 'var canvas = document.getElementById("canvas");';
            echo 'canvas.onclick = function (evt) {var activePoints = myLine.getElementAtEvent(evt);';
