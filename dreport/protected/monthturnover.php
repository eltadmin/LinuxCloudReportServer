...
<?php
	...
	$C_SQL .= '"LastTurnover":{"Type":"Query","SQL":"select EXTRACT(MONTH FROM cast(S.SELL_DATETIME - cast(TIMEOFFSET as float)/24 as date)) \"SELL_DATE_MONTH\", EXTRACT(YEAR FROM cast(S.SELL_DATETIME - cast(TIMEOFFSET as float)/24 as date)) \"SELL_DATE_YEAR\", sum(S.SELL_SUM) \"SELL_SUM\", C.CURR_SIGN from SALES_BON S inner join N_CURRENCY C on S.SELL_CURRTYPE = C.CURR_ID where (cast(S.SELL_DATETIME - cast(TIMEOFFSET as float)/24 as TIMESTAMP) between START_DATE and END_DATE) and(S.SELL_REVOKED_ = 0) group by 1, 2, 4 order by 2, 1 asc"},';    
    $C_SQL .= '"CurrentPayType":{"Type":"Query","SQL":"select P.PAY_TYPE, T.PAYTYPE_NAME, SUM(P.PAY_SUM) \"PAY_SUM\" from SALES_PAYMENT P inner join SALES_BON S on P.PAY_SELL_ID = S.SELL_ID inner join N_PAYMENT_TYPES T on P.PAY_TYPE = T.PAYTYPE_NUMB where (cast(S.SELL_DATETIME - cast(TIMEOFFSET as float)/24 as TIMESTAMP) BETWEEN PAYTYPE_DT_START and END_DATE)and(S.SELL_REVOKED_ = 0) group by 1, 2 order by 1 asc"}}';
              ...
              if ($rptdata->ResultCode == 0) {
				  ...
				  if (is_array($rptdata->LastTurnover) && !empty($rptdata->LastTurnover)) {
					$TotalTurnover = (float)end($rptdata->LastTurnover)->SELL_SUM;
						...
						$TotalTurnover = 0; //  LastTurnover is empty 
					}
                  ...
                  if ($ItemsCount > 0) {
					...
					$AvgTurnover = 0; //  ItemsCount is zero
				  }
...
*/		
       ...
       if ($_SESSION['lang'] == 'bg'){
         echo '&nbsp;•&nbsp;<a href="monthturnover.php?lang=en"><img src="images/en.png" /></a>';
       } else {
         echo '&nbsp;•&nbsp;<a href="monthturnover.php?lang=bg"><img src="images/bg.png" /></a>';
       }
