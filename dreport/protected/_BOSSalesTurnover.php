...
<?php
    ...
    $C_SQL .= '"CurrentPayType":{"Type":"Query","SQL":"SELECT DOCUMENT_NAME, SUM(SUP_SUM) \"SUP_SUM\", CURR_SIGN FROM SUPPLIES JOIN N_CURRENCY ON SUP_CURRENCY = CURR_ID JOIN N_DOCUMENTS ON SUP_DOCTYPE = DOCUMENT_ID WHERE SUP_TYPE = 2 AND CAST (SUP_DATETIME - CAST(TIMEOFFSET AS FLOAT)/24 AS DATE) >= CURRENT_DATE AND SUP_REVOKED_ = 0 GROUP BY 1,3 ORDER BY 1"}}';
      ...
      <tr>
           ...
           <?php 
            if ($expiredate >= time()){		   		   
				...
				echo '" class="medium green button"><<</a></td>';
			} else {
			    echo '<td align="center"><a href="#" onclick="show_exp()" class="medium green button"><<</a></td>';				
			}  		   
           ?> 
        
        <td align="center" style="width:60%;"><a href="rptlist.php" class="medium purple button" style="width:95%;"><?php echo $lang["btnExit"]; ?></a></td> 
           <?php 
            if ($expiredate >= time()){		   		   
			   echo '<td align="center"><a href="';
				$today = new DateTime(); 
